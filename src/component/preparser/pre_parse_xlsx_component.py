...
class PreParseXlsxComponent(object):
    ...
    def pre_parse(self):
        ...
        sheet = book.active
        index = 0
        horizontal = self.direct_detection(sheet)
        logger.info("basic direction: %s" % ("horizontal" if horizontal else "vertical"))
        for row in sheet:
            index += 1
            next_row = None
            if index < sheet.max_row:
                next_row = sheet[index+1]
            ...
            try:
                ...
                if traversed_title:
                    if not self.ctx.bank_name and not self.ctx.bank_account:
                        self._parse_from_content(sheet, row)
                    break
                ...
                if join_info and not self.ctx.bank_name:
                    ...
                    if self.ctx.bank_name == '':
                        self.ctx.bank_name = self._parse_field(row_val, kw_bank_name_d, join_info, next_row, horizontal)
                    self.ctx.bank_name = re.sub('[^\u4e00-\u9fa5]', '', self.ctx.bank_name)
                if not self.ctx.bank_account:
                    self.ctx.bank_account = self._parse_field(row_val, x_kw_bank_account, join_info, next_row, horizontal)
                    self.ctx.bank_account = re.sub('[\u4e00-\u9fa5]', '', self.ctx.bank_account)
                if not self.ctx.user_name:
                    self.ctx.user_name = self._parse_field(row_val, kw_user_name, join_info, next_row, horizontal)
                    self.ctx.user_name = re.sub('[^\u4e00-\u9fa5]', '', self.ctx.user_name)
                if not self.ctx.start_date:
                    self.ctx.start_date = self._parse_field(row_val, kw_start, join_info, next_row, horizontal)
                if not self.ctx.end_date:
                    self.ctx.end_date = self._parse_field(row_val, kw_end, join_info, next_row, horizontal)
                ...
                if not self.ctx.row_count:
                    self.ctx.row_count = self._parse_field(row_val, kw_row_count, join_info, next_row, horizontal)
    ...
    @decl_field_val
    def _parse_field(self, row_val, kw_info, join_info, next_row, horizontal=True):
        ...
        for k in kw:
            ...
            for val in row_val:
                col_index += 1
                if matched and horizontal:
                    if val:
                        return val
                    continue
                if type(val) is str and re.search(k, val) and self._is_exclude(val, ex_kx):
                    matched = True
                    if not horizontal and next_row:
                        return next_row[col_index].value
        return ''
    ...
    def _parse_from_content(self, sheet, row):
        usr_name_index = -1
        account_no_index = -1
        row_index = -1
        for cell in row:
            ...
            if cell.value in x_kw_bank_account_d:
                account_no_index = cell.column
            elif cell.value in x_kw_usr_name_d:
                usr_name_index = cell.column
        ...
        for row in sheet:
            ...
            if account_no_index >= 0 and not self.ctx.bank_account:
                self.ctx.bank_account = row[account_no_index-1].value
            if usr_name_index >= 0 and not self.ctx.user_name:
                self.ctx.user_name = row[usr_name_index-1].value

    def direct_detection(self, sheet):
        try:
            for i in range(1, trans_header_max_rows):
                row = sheet[i]
                index = 0
                for cell in row:
                    index += 1
                    if cell and cell.value:
                        if len(list(filter(lambda e: re.fullmatch(e, str(cell.value)), x_kw_bank_account_d))) > 0:
                            right_val = sheet[i][index].value
                            return re.fullmatch(r'[a-zA-Z\d]+', right_val)
        except Exception as e:
            logger.error(e)
        return True
